...
export const getCampById = async (id) => {
  const result = await pool.query("SELECT * FROM camps WHERE campId = $1", [id]);
  return result.rows[0];
};

export const createCamp = async (campData) => {
  const { title, description, location, age_range, mentor, category, capacity } = campData;
...
export const deleteCampById = async (id) => {
  const result = await pool.query("DELETE FROM camps WHERE campId = $1 RETURNING *", [id]);
  return result.rows[0];
};